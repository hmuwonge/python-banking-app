...
class UserCreationForm(DjangoUserCreationForm):
    class Meta:
        model = User
        fields=["email","id_no","first_name","middle_name","last_name",
                "security_question","security_answer","is_staff","is_superuser"]
...
class UserChangeForm(DjangoUserChangeForm):
    ...
    def clean_email(self):
        ...
        if User.objects.exclude(pk=self.instance.pk).filter(email=email).exists():
            raise ValidationError(_("A user with the same email already exists"))
        return email

    def clean_id_no(self):
        id_no= self.cleaned_data.get("id_no")
        if User.objects.exclude(pk=self.instance.pk).filter(id_no=id_no).exists():
            raise ValidationError(_("A user with the same ID number already exists"))
        return id_no
